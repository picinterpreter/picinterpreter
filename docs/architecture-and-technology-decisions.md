...
本节记录已经在产品讨论中明确下来的决策。它们不是最终实现细节，但应作为后续 ADR、issue 和代码实现的产品边界。

### 10.1 离线双向沟通必须可用

- 决策：没有网络时，核心双向沟通必须可用。
- 理由：图语家是沟通辅助工具，患者和照护者不能因为网络不可用而失去核心沟通能力。
- 证据：已在 issue `#32` 中明确：网络可以提升质量，但不能决定用户能不能沟通。
- 生效范围：
  - 患者表达：本地图、选图、排序、模板句、大字展示、浏览器 TTS best-effort
  - 接收理解：手动输入、本地分词、本地图匹配、修正图片序列、全屏展示
  - 历史记录：双向记录本地保存
  - 本地/自定义图片：离线继续可用

网络依赖能力只能作为增强，包括 LLM 造句、LLM 重分词、在线补图、云同步、登录和后端 ASR。

### 10.2 AI 是润色和补救，不是主控制器

- 决策：AI 只用于润色、补救和低置信度场景，不默认接管图片匹配流程。
- 理由：AAC 场景更重视意思准确和可解释性，而不是让 AI 看起来聪明。
- 证据：已在 issue `#8` 中明确：AI 是 enhancer / fallback，不是 steering wheel。
- 生效范围：
  - 图匹配不到时，AI 可以协助补救
  - 语句顺序混乱时，AI 可以帮助归一化
  - 表达端候选句可以由 AI 润色
  - 接收端最终展示给患者的图片序列必须允许照护者确认和修正

### 10.3 接收端修正者是照护者

- 决策：接收端图片序列的修正者是家属或照护者，不是患者。
- 理由：接收端处理的是照护者自己说的话或输入的文字，患者不应承担纠错任务。
- 证据：已在 issue `#26` 中明确 receiver-side correction 的产品含义。
- 生效范围：
  - ReceiverPanel 匹配结果编辑
  - 全屏展示前的确认流程
  - ReceiverCorrection 日志与修正记忆

### 10.4 接收端记录采用两阶段保存

- 决策：接收端先保存初稿，再用照护者确认后的最终图片序列覆盖同一条记录。
- 理由：既保留原始输入，又保证历史记录代表最终展示给患者的结果。
- 证据：issue `#26` 已将这一点定义为根因修复方向。
- 生效范围：
...
- 决策：照护者修正不只影响本次会话，还影响当前账号；如果是家庭账号，则影响整个家庭工作区。
- 理由：同一个账号或家庭反复修正同一表达时，系统应减少重复劳动。
- 证据：已在 issue `#26` 中明确修正记忆的账号 / 家庭作用域。
- 生效范围：
  - ReceiverCorrection
  - 个性化词典或 mapping override
  - 家庭账号共享偏好

单个账号或家庭的修正不能立即变成全局规则。全局规则只能从多账号、多家庭的修正模式中聚合和审核后推导。

### 10.6 未命中词采用分阶段补图流程

- 决策：未命中词先自动在线搜图；在线搜索无法补图时，进入待维护列表，由照护者决定如何处理。
- 理由：系统应先降低照护者负担，但最终维护权仍归照护者。
- 证据：已在 issue `#19` 中明确 staged recovery flow。
- 生效范围：
  - 未命中词列表
  - 在线图库搜索
  - AI 生图
...
补图时系统可以智能预填标签、同义词、词性、语义属性和来源信息，但保存前应由照护者审核。

### 10.7 私人图片允许同步，但必须可控

- 决策：私人或自定义图片可以同步到账号或家庭工作区，但每张图片都应有隐私控制。
- 理由：真实 AAC 图库常需要个人照片，但这些照片的同步、导出和分享边界必须由照护者控制。
- 证据：已在 issue `#10` 中明确 private/custom picture privacy decision。
- 生效范围：
  - 自定义图片上传
  - 家庭账号同步
  - 图库导出
...
- `local only`：仅本地保存，不同步，不导出
- `sync allowed`：允许在账号或家庭工作区内同步
- `export allowed`：导出时在照护者选择包含私人图片的情况下可导出

### 10.8 图片和词条元数据需要语言属性

- 决策：先增强图片 / 词条元数据，而不是立即重构图片序列结构。
- 理由：匹配、排序和上下文理解需要知道词语角色；但直接替换 `pictogramIds: string[]` 会牵动过大。
- 证据：已在 issue `#11` 中明确 pictogram metadata direction。
- 生效范围：
  - PictogramEntry
  - 词库和同义词表
  - 接收端匹配与排序
...
- 决策：图库按可复用图片集设计，而不是只做固定分类或图片列表。
- 理由：照护者和治疗师需要复用、迁移和维护已有 AAC 资产。
- 证据：已在 issue `#11` 和 `#30` 中明确结构化图库和 CBoard / OBF 导入方向。
- 生效范围：
  - 分类管理
  - 图片集链接
  - CBoard / OBF 导入
...
例如“玩具”图片集可以同时被学校、家里、物品等场景引用；维护一次，引用处自动受益。

### 10.10 CBoard / OBF 导入是迁移能力

- 决策：CBoard / OBF 导入应视为迁移能力，而不是普通小工具。
- 理由：用户和治疗师可能已经拥有重要 AAC 图库和板面，图语家应降低迁移成本。
- 证据：已在 issue `#30` 中明确 import as migration capability。
- 生效范围：
  - 导入预览
  - 隐私照片检查
  - 来源和授权元数据
...
### 10.11 账号模型支持个人账号和家庭账号

- 决策：长期账号模型应支持个人账号和家庭账号。
- 理由：真实照护通常不是单人完成，一个患者可能有多个家属或照护者共同参与。
- 证据：已在 issue `#31` 中明确 personal account / family account direction。
- 生效范围：
  - 认证
  - 同步
  - 上下文和修正记忆
...
家庭账号可绑定约 5 到 6 个用户或照护者，共享同一个照护工作区。架构上不要把 `userId` 永久等同于 `patientId`。

### 10.12 本地数据库升级是发布风险

- 决策：每次改本地数据库结构，都必须验证老用户升级路径。
- 理由：图语家的本地数据就是用户的沟通资产，升级失败会直接影响沟通能力。
- 证据：已创建 issue `#35` 跟踪 IndexedDB / Dexie migration strategy。
- 生效范围：
  - Dexie schema version
  - Service Worker 发布
  - 新表如 `receiverCorrections`
  - 老用户升级测试

### 10.13 患者端按钮图标优先

- 决策：患者端核心操作尽量使用图标和图片，不依赖患者阅读文字。
- 理由：失语症和认知负担场景下，文字按钮不能作为唯一操作入口。
- 证据：已在 issue `#6` 中明确 icon-first patient controls。
- 生效范围：
  - 选图、删除、排序
  - 播放、停止、收藏
  - 返回、关闭
...
文字可以作为无障碍标签、tooltip 或照护者侧说明，但患者主流程应图标优先。

### 10.14 测试要验证真实沟通流程

- 决策：测试不只验证函数和构建，还必须验证真实患者 / 照护者流程。
- 理由：AAC 产品的关键风险在触控、TTS、ASR、离线和横竖屏，而不是单个函数。
- 证据：已在 issue `#33` 中明确 acceptance testing scope。
- 生效范围：
  - Playwright E2E
  - 真机验收
  - 版本发布 checklist

必须覆盖患者选图到播报、照护者输入到全屏展示、断网双向流程、触屏拖拽、TTS 声音和横竖屏展示。

### 10.15 图源授权和隐私属于图片元数据

- 决策：每张图片都应保留来源、作者、许可证、是否可商用、是否需要署名、是否私人照片等信息。
- 理由：图源授权和隐私会直接影响导出、分享、商业化和家庭使用边界。
- 证据：已在 issue `#24` 中明确 library metadata decision。
- 生效范围：
  - PictogramSource
  - 图库导入
  - 图库导出
...
### 10.16 暂不引入复杂 CRDT 协同

- 决策：当前不引入复杂 CRDT 协同，先使用简单同步和冲突提示。
- 理由：当前 MVP 和家庭账号场景尚不需要多人高频离线共同编辑同一图库的复杂度。
- 证据：架构讨论已形成共识：不提前为假设规模过度设计，但不能堵死未来协同路径。
- 生效范围：
  - 同步策略
  - 家庭账号协作
  - 冲突提示

当前目标是不静默覆盖关键数据。未来若多人高频共同维护同一图库，再重新评估 CRDT。
