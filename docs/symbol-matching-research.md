...
更新时间：2026-05-02（初稿）；2026-05-02（补充架构澄清、中文消歧工具、Phase 1/2 方案）；2026-05-07（补充学术研究、开源管道、Codex 综合判断）；2026-05-08（补充 descriptor 重叠词冲突说明）
...
### 14.7 Seed 策展注意事项：避免 descriptor 与旧条目重叠

新增 `descriptors` 这类描述词板块时，要特别注意不要和旧 seed 条目共享同一个精确标签或高风险同义词。
...
`DOMAIN_MATCH_BONUS = +40` 只能在**同阶段内部**起作用，而且前提是 lexicon 里存在对应 `category`。  
所以如果两个 pictogram 同时是：

- `exact`
- 或同时是 `synonym`

又没有额外 tie-break，那么通常是谁先出现在 seed 数组里，谁就赢。

这类问题不会直接报错，但会造成一种很隐蔽的行为冲突：

- UI 上看起来新概念已经加进来了
- 实际输入这个词时，系统还是更常落到旧概念
- 于是新概念“看起来存在，实际上不稳定可达”
...
- `p_right` 和 `d_right` 若都吃 `右边`
- 都是 exact `400`
- 旧条目会稳定压住新 descriptor
...
这会让 descriptor 层里的 `多` 看起来有了，但长词短语入口并不真正属于它。
...
在没有更强上下文解释器的前提下，这类重叠会让输入结果不稳定。

#### V1 的推荐修法

V1 最稳妥的做法不是立刻改 matcher 核心打分，而是先把 seed 边界收干净：

- descriptor 保留最短、最稳定、最不冲突的词入口
- 去掉和旧条目重叠的 exact label
- 去掉高风险同义词短语
- 先让旧条目继续承接那些更成熟、已稳定存在的长词短语
...
> 除非同时引入显式 tie-break 规则，否则要避免 seed 里出现“新 descriptor 条目”和“旧条目”共享完全相同的精确词或高风险同义词。
