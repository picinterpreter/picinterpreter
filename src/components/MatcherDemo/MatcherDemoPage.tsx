...
function TokenCard({ match }: { match: MatchedToken }) {
  ...
  return (
    <div className={`flex flex-col items-center gap-1 p-2 rounded-xl border-2 w-24 flex-shrink-0 ${
      isMatched ? 'border-blue-200 bg-white' : 'border-dashed border-gray-200 bg-gray-50'
    }`}>
      <img
        src={imgSrc}
        alt={token}
      ...
      />
      <span className="text-sm font-medium text-gray-800 truncate w-full text-center">{token}</span>
...
function MetricsBadge({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="flex flex-col items-center bg-white rounded-xl border border-gray-200 px-4 py-3 min-w-[80px]">
...
export function MatcherDemoPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <header className="flex items-center justify-between px-4 py-3 bg-blue-700 text-white">
        <div>
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-6">
        ...
        <section className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
          <h2 className="font-semibold text-gray-700">输入文本</h2>
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input
              ...
              className="flex-1 border border-gray-300 rounded-xl px-4 py-2 text-lg focus:outline-none focus:border-blue-500"
              autoFocus
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="px-5 py-2 bg-blue-600 text-white rounded-xl font-medium disabled:opacity-40 hover:bg-blue-700 transition-colors"
            >
              {loading ? '分析中…' : '分析'}
            </button>
          </form>
        ...
        {result && (
          <>
            ...
            <section className="space-y-2">
              <h2 className="font-semibold text-gray-700">指标</h2>
            ...
            {/* Segmentation tokens */}
            <section className="bg-white rounded-2xl border border-gray-200 p-4 space-y-2">
              <h2 className="font-semibold text-gray-700">分词结果</h2>
              <div className="flex flex-wrap gap-2">
            ...
            {/* Image sequence */}
            <section className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
              <h2 className="font-semibold text-gray-700">图片序列</h2>
