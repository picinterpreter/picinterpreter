...
- **标签**：路线参考；与当前图语家接收端「句子 → 结构 → 图符序列」的思路有共鸣
...
这两个仓库一起形成目前唯一可找到的完整开源 text→WSD→pictogram 管道：
...
**直接可借鉴**
- `getalp/Arasaac-WN` + `getalp/disambiguate`：英文侧完整管道，Phase 2 技术原型
- `thunlp/SememeWSD`：中文词义消歧的唯一起点，与 OpenHowNet 同源
- ARASAAC 已有中文标签：API 搜索直接支持中文词，不需要另建映射层
- Colourful Semantics WHO/DOING/WHAT 分层：对接收端拆句子有设计参考价值

**路线参考**
- ICON 2024 双向框架：语言无关 + 有数据集是亮点，代码未确认，Phase 2 参考
- CWN → WordNet → Arasaac-WN 链路：可行但需自行拼接，Phase 2 首要研究任务
- Pictogrammar 语义文法：替代路线，从句子结构而非逐词消歧出发
